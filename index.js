window.onload = function () {
    document.getElementsByTagName("mdui-card")[0].style.display = "block"
    document.querySelector("body > mdui-layout > mdui-top-app-bar > mdui-top-app-bar-title").innerText = "王者自定义房间 3.0"
}
...
if (localStorage.getItem("wzzdy_xgluatip") != "0.1") {
    ...
    // 设置<span>元素的文本内容  
    span.innerHTML = '<span slot="description">全新版的lua修改脚本已经上线 使用脚本可以实现直接邀请好友(不用滴滴) 以及不用10人开房间 添加人机 以及可以像主播一样设置房间(仅限5v5) 完全免费 理论支持正式服/体验服 点击下方我知道了 查看教程<br>提示 ：开启脚本有风险 请酌情开启 仅支持安卓</span>';
    mdui.alert({
        headline: "提示",
        description: span,
        confirmText: "我知道了",
...
async function initMypage() {
    ...
    document.querySelectorAll(".myedit")[1].onclick = function () {
        var dialog = document.querySelector(".example-dialog")
        dialog.open = true
    }
    ...
    for (item in mydatajson[1]) {
        // 使用闭包解决
        (function (item_str) {
            var listItem = document.createElement('mdui-chip');
            listItem.textContent = item_str;
            listItem.selectable = true;
            listItem.className = "mychip"
            // 将新创建的元素添加到 DOM 中，例如添加到 body 中  
            document.getElementsByClassName("myherolist")[0].appendChild(listItem);
        })(item);
    }
